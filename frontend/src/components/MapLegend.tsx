...
/** Compact persistent map legend — vehicle dash identity + delivery semantics. */
export function MapLegend({
  ...
  return (
    <aside
      className="pointer-events-auto absolute bottom-3 left-3 z-[500] max-w-[220px] border border-ink bg-snow/95 px-3 py-2.5 shadow-sm backdrop-blur-[2px]"
    ...
    >
            ...
            <li key={id}>
              ...
              >
                <span
                  ...
                  aria-hidden
                />
                <span className={`font-mono text-[11px] font-semibold ${down ? "text-mute line-through" : "text-ink"}`}>
                  {id}
                </span>
      ...
      <ul className="mt-1.5 space-y-1">
        <StatusRow mark={<span className="inline-block h-2.5 w-2.5 rounded-full border-2 border-ink bg-snow" />} label="Normal" />
